      ...
      <Tabs defaultValue="open" className="w-full min-h-0 flex-1 flex flex-col">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="open" className="gap-1.5 text-xs">
            ...
            Öppna
            {open.length > 0 && <Badge variant="destructive" className="text-[9px] h-4 px-1">{open.length}</Badge>}
          </TabsTrigger>
          <TabsTrigger value="resolved" className="gap-1.5 text-xs">
            <CheckCircle2 className="w-3.5 h-3.5" />
            Lösta
            <Badge variant="outline" className="text-[9px] h-4 px-1">{resolved.length}</Badge>
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="open" className="mt-3 min-h-0 flex-1">
          {open.length === 0 ? (
            ...
            <ScrollArea className="h-full min-h-0">
              <div className="space-y-2 pr-2">{open.map(r => renderBugCard(r))}</div>
            </ScrollArea>
          )}
        </TabsContent>
        ...
        <TabsContent value="resolved" className="mt-3 min-h-0 flex-1">
          {resolved.length === 0 ? (
            ...
            <ScrollArea className="h-full min-h-0">
              <div className="space-y-2 pr-2">{resolved.map(r => renderBugCard(r))}</div>
            </ScrollArea>
          )}
        </TabsContent>
      </Tabs>
