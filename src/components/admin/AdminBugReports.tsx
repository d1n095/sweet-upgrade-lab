    ...
    <div className="flex h-full min-h-0 flex-col space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <Badge variant={openReports.length > 0 ? 'destructive' : 'secondary'} className="text-xs">{openReports.length} öppna</Badge>
      ...
      <Tabs defaultValue="open" className="w-full min-h-0 flex-1 flex flex-col">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="open" className="gap-1.5 text-xs">
            ...
            Öppna
            {openReports.length > 0 && <Badge variant="destructive" className="text-[9px] h-4 px-1">{openReports.length}</Badge>}
          </TabsTrigger>
