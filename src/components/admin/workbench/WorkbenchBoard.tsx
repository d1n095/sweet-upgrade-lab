...
const WorkbenchBoard = ({ initialFilter }: Props) => {
  ...
  const runAutomation = async () => {
    ...
    try {
      const { data, error } = await safeInvoke({ action: 'RUN_AUTOMATION_ENGINE', fn: 'automation-engine' });
  ...
  const runOrchestrator = async () => {
    ...
    try {
      const { data, error } = await safeInvoke({ action: 'RUN_AI_ORCHESTRATOR', fn: 'ai-task-manager', body: { action: 'orchestrate' } });
      if (error) throw error;
      const r = data?.results;
