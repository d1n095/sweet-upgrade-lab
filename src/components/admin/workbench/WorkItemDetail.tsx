...
import { Sparkles, Tag, Copy, Loader2 as Loader2Icon, EyeOff, RotateCcw, PenLine, ChevronDown, ChevronUp, Radar } from 'lucide-react';
...
const WorkItemDetail = ({ item, open, onOpenChange, onStatusChange, onRefresh }: WorkItemDetailProps) => {
  ...
  const handleIgnore = async () => {
    ...
    try {
      ...
      onRefresh?.();
      onOpenChange(false);
    } catch (e) { toast.error('Fel vid ignorering'); void e; }
  ...
  const handleRunRootCause = async () => {
    toast.info('Rotorsaksanalys körs via skanning — se skannerresultat');
  };
      ...
      <SheetContent side="right" className="w-[440px] sm:w-[520px] p-0 flex flex-col h-full min-h-0">
        ...
        <ScrollArea className="flex-1 min-h-0">
          <div className="p-5 space-y-5">
            ...
            {/* Bug report metadata (scanner-processed) */}
            ...
            {bugData && item.item_type === 'bug' && (
              <div className="space-y-2">
                ...
                {item.human_selected_cause && (
                  <div className="border border-accent/30 rounded-lg p-2.5 bg-accent/5 text-xs">
                    <span className="font-medium text-accent">Vald orsak:</span> {item.human_selected_cause}
                  </div>
                )}
              </div>
            )}
                  ...
                  <Button size="sm" variant="outline" className="flex-1 gap-1 h-7 text-xs text-destructive border-destructive/30"
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        toast.info('🔍 Avvisad — prioritet eskalerad');
                        onRefresh?.();
                      } catch { toast.error('Fel'); }
                      finally { setRunningPreVerify(false); }
                    }}
            ...
            {/* Pre-Verify Button for open items — removed (no AI) */}
