...
import { EyeOff, RotateCcw, PenLine, ChevronDown, ChevronUp } from 'lucide-react';
            ...
            {bugData && item.item_type === 'bug' && isOpen && (
              <div className="space-y-2">
                <Button size="sm" variant="ghost" className="w-full h-7 text-xs gap-1.5" onClick={() => setShowCustomCause(!showCustomCause)}>
                  <PenLine className="w-3 h-3" /> {showCustomCause ? 'Dölj orsak' : 'Skriv orsak / fix'}
                </Button>
                {showCustomCause && (
