...
const SystemExplorer = () => {
  ...
  const scanSourceCount = workItems.filter((w) => w.source_type === "scan" || w.source_type === "scan_result").length;
  ...
  const groupedScannerStats = useMemo(() => {
    const rawIssues = (scanResults?.issues as any[] | undefined) ?? [];
    const scanItems = workItems.filter(w => w.source_type === "scan" || w.source_type === "scan_result" || w.source_type === "scan_detection");
                                        ...
                                        <div className="flex items-center gap-1 flex-wrap">
                                          ...
                                          {issue._viewport && <Badge variant="outline" className="text-[8px] px-1 py-0">📱 {issue._viewport}{issue._viewport_width ? ` (${issue._viewport_width}px)` : ''}</Badge>}
                                           {issue._affected_area && <Badge variant="outline" className="text-[8px] px-1 py-0">📍 {issue._affected_area.type}/{issue._affected_area.target}</Badge>}
                                            {issue._origin_source && <Badge variant="outline" className="text-[8px] px-1 py-0">{issue._origin_source === "scan_result" ? "🔍" : issue._origin_source === "manual" ? "👤" : "🔧"} {issue._origin_source}</Badge>}
          ...
          {detailTab === "info" && (
            <div className="space-y-3 text-sm">
              ...
              <div>
                ...
                <p>
                  <Badge variant="outline" className="text-[10px]">
                    {selectedItem.source_type === "scan" || selectedItem.source_type === "scan_result" || selectedItem.source_type === "scan_detection"
                      ? "🔍 scan"
              ...
              <div>
                <span className="text-muted-foreground text-xs">Scan Detected</span>
