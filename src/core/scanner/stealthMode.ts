 ...
 * STEALTH MODE
 *
 * Runs background architecture/validation work without any visible UI impact:
 *  - schedules tasks via requestIdleCallback (falls back to a low-rate timer)
 *  - never blocks the main thread (one task per idle slice, with deadline)
 *  - never mutates the DOM, never logs to console (writes go to systemStateRegistry)
 *  - auto-disables itself if instability is detected (errors threshold or
 *    long-task threshold exceeded)
 *
 * UI surfaces (overlay, mini workbench, dashboards) read this store and hide
 * themselves while stealth is ACTIVE.
 *
 * Access from devtools (debug trigger):
 *   window.__godmode.enableStealth()
 *   window.__godmode.disableStealth()
 *   window.__godmode.status()
 *   window.__godmode.logs()
 */
...
export interface BackgroundTask {
  key: ModuleKey;
  /** Returns the value to store; must be cheap or chunked. */
...
class _StealthScheduler {
  ...
  private async tick(deadline: { timeRemaining: () => number }) {
    ...
    // Reschedule next idle slice (1.5s minimum gap so we never spike CPU)
...
// ── Default background tasks (cheap, derived only) ───────────────────
