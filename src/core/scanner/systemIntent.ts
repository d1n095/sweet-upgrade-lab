/**
 * SYSTEM INTENT ENGINE
 *
 * Deterministic next-action predictor. NO AI, NO probability.
 * Pure pattern-match against:
 *   - current_state  (latest slot values)
 *   - previous_state (last snapshot held in memory)
 *   - execution_log  (recent commandLayer entries)
 *
 * Each rule is an explicit IF / THEN. If no rule matches → empty list.
 * Suggestions are advisory; engine never executes them.
 */
...
const RULE_repeated_violations: Rule = (cur, prev) => {
  ...
  // "Repeated" = at least 2 new rejections vs. previous snapshot, OR
  // rejection_count grew at all when previous was already > 0.
...
/**
 * Compute next-action suggestions deterministically.
 * Each call advances the "previous" snapshot pointer.
 */
...
/** Reset history (e.g. after a heal/refactor cycle). */
